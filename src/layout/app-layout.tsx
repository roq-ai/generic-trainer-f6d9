...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['trainer'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<TrainerInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/trainers` : null),
    () => getTrainers({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Trainer',
      path: isTenantUser ? `/trainers/view/${data?.[0]?.id}` : '/trainers',
      entity: 'trainer',
      service: AccessServiceEnum.PROJECT,
      icon: FiUserCheck,
    },
    { name: 'Course', path: '/courses', entity: 'course', service: AccessServiceEnum.PROJECT, icon: FiBookOpen },
    { name: 'Exercise', path: '/exercises', entity: 'exercise', service: AccessServiceEnum.PROJECT, icon: FiActivity },
    { name: 'Quiz', path: '/quizzes', entity: 'quiz', service: AccessServiceEnum.PROJECT, icon: FiClipboard },
    { name: 'Topic', path: '/topics', entity: 'topic', service: AccessServiceEnum.PROJECT, icon: FiBook },
    { name: 'Use Case', path: '/use-cases', entity: 'use_case', service: AccessServiceEnum.PROJECT, icon: FiFileText },
